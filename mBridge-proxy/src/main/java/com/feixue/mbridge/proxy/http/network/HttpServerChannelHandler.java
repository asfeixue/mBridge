...
import com.feixue.mbridge.proxy.http.HttpInvoker;
import com.feixue.mbridge.proxy.http.filter.CheckFilter;
import com.feixue.mbridge.proxy.http.filter.StoreFilter;
import com.feixue.mbridge.proxy.http.filter.TransformFilter;
 ...
 */
public class HttpServerChannelHandler extends SimpleChannelInboundHandler<FullHttpRequest> implements Channel {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest msg) throws Exception {
        ...
        filterChain.addLast(new TransformFilter());
        filterChain.addLast(new StoreFilter());
        filterChain.addLast(new CheckFilter());

        Invoker invoker = new HttpInvoker(ctx, filterChain);
        Invocation invocation = new Invocation();
        invocation.setContent(msg);

        outbound(invoker, invocation);
...
//        }
    ...
    public void inbound(Invoker invoker, Invocation invocation) {
        HttpInvoker httpInvoker = (HttpInvoker) invoker;
    }
